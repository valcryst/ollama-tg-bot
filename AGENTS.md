...
Telegram bot backed by **Ollama**, with a **React dashboard** for configuration. One Node process runs the Grammy bot, Express API, and (in production) serves the built dashboard.

| Workspace | Role |
|-----------|------|
| `server/` | Bot, Ollama client, SQLite, REST API |
...
Ollama host, model, prompts, owner, and performance limits live in **dashboard settings** (SQLite), not `.env`.

## Architecture

```
Telegram → Grammy handlers → chat-turn → Ollama
                ↓
...
- Client: `server/src/ollama/client.ts`
- Chat options: `server/src/settings-limits.ts` (`num_ctx`, `num_predict`, `temperature`)
...
- Compression when large: `server/src/context-compress.ts`
...
| `/settings` | Ollama, model, owner, performance, vision |
